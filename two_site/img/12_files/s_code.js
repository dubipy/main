...
var s_domain = window.document.location.host;
if((s_domain.indexOf("dev")!=-1) || (s_domain.indexOf("beta")!=-1) || (s_domain.indexOf("test")!=-1) || (s_domain.indexOf("stage")!=-1) ||
(s_domain.indexOf("local")!=-1) ||
(s_domain.indexOf("edit")!=-1) ||
(s_domain.indexOf("qa")!=-1) ||
(s_domain.indexOf("secondscreentrivia")!=-1)
){
	s_account = "nbcuusanetworkddev";
}
else{
...
s.linkInternalFilters="javascript:,usanetwork.com,characterbrandworks.com,charactersunite.com,characterblog.com,suitsoncampus.com,westminsterkennelclub.org,htk2.theatrics.com,samsung.com,twitter.com/intent,whoismrrobot,colonytv.com";
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,gif,jpg"
...
function s_doPlugins(s) {
	...
	//disable auto download link tracking on characterbrandworks.com,
	if(document.URL.match('characterbrandworks.com')){s.trackDownloadLinks=false}
	...
	//Props
	if (typeof(usa_clubOmnitureProp1) != 'undefined') {s.prop1 = usa_clubOmnitureProp1}
	s.prop1=s.getAndPersistValue(s.prop1,'s_prp1');
	...
	//New or old site
	// if(!s.prop32) {s.prop32="Old Site"};
	if (s.eVar68) {s.prop32="New Site"}; //evar68 is only used on pages with responsive design
	if(s.prop6.match('iframe')){s.prop32="New Site"};// iframed content that shows within responsive design
	...
	if(s.pageName) {s.eVar9=s.pageName;}
	...
	s.exit=s.exitLinkHandler();
		//if(s.exit){
		//s.eVar24 = s.getQueryParam('o',':',s.exit);
		//s.campaign=s.getQueryParam('cid,__source',':',s.exit);
		//s.linkTrackVars='eVar24,campaign';}

		if ((s.exit.indexOf("double")>-1) && document.URL.match('classaction'))
	{
		console.log('double exit on classaction');
		s.trackExternalLinks=false;
	}

	if (s.exit.indexOf("evr.st")>-1) {
		s.linkTrackVars="prop31";
		s.prop31="Everest Exit Link";
		s.linkName="Everest Exit Link";
	}

	if (s.exit.indexOf("facebook.com/TheMomentUSA")>-1){
		s.linkTrackVars="events,eVar74";
		s.linkTrackEvents=s.events="event41";
		s.eVar74="Facebook";
		s.linkName="Social Share";
	}

	if (s.exit.indexOf("twitter.com/TheMoment_USA")>-1){
		s.linkTrackVars="events,eVar74";
		s.linkTrackEvents=s.events="event41";
		s.eVar74="Twitter";
		s.linkName="Social Share";
	...
	if (s.exit.indexOf("27646")>-1) {
		s.linkTrackVars="prop31";
		s.prop31="Modern Family : FamCam : Target Exit Link";
		s.linkName="Modern Family : FamCam : Target Exit Link";
	}

	if(s.exit.indexOf("share?channel=facebook-share")>-1) {
		s.linkTrackVars='events,eVar74';
		s.linkTrackEvents=s.events='event41';
		s.eVar74='Facebook';
	...
	if(s.exit.indexOf("share?channel=twitter")>-1) {
		s.linkTrackVars='events,eVar74';
		s.linkTrackEvents=s.events='event41';
		s.eVar74='Twitter';
		s.manageVars('clearVars',s.linkTrackVars,1);
	...
	// Lexus exit link in Suits Recruits: Off the Clock
	if(s.exit.indexOf("274208955;99984142;b")>-1) {
		s.linkTrackVars='prop31';
		s.prop31='Lexus Exit Link';
		//s.manageVars('clearVars',s.linkTrackVars,1);
	}
...
if (typeof s != 'undefined') {
  ...
  if (typeof(document.documentElement.clientWidth) != 'undefined') {
    ...
    if (usa_cw <= 644) {
      s.eVar68 = usa_deviceInfo.mobileDevice ? 'Smartphone Portrait' : 'Desktop';
    } else if (usa_cw >= 645 && usa_cw <= 959 ) {
      s.eVar68 = usa_deviceInfo.mobileDevice ? 'Tablet Portrait' : 'Desktop';
    } else if (usa_cw >= 960 && usa_cw <= 1274 ) {
      s.eVar68 = usa_deviceInfo.mobileDevice ? 'Tablet Landscape' : 'Desktop';
    } else if (usa_cw >= 1275 ) {
      s.eVar68 = 'Desktop Large';
    }
  }
  // Desktop Large | Desktop | Tablet Landscape | Tablet Portrait | Smartphone Portrait

  // set screen height
  s.eVar69 = typeof(document.documentElement.clientHeight) != 'undefined' ? document.documentElement.clientHeight : '';

  // set screen width
  s.eVar70 = typeof(document.documentElement.clientWidth) != 'undefined' ? document.documentElement.clientWidth : '';
...
//copying over evar 68 -> 72 in to prop68 - > 72
	...
	if(s.eVar71) {s.prop71="D=v71"};
	if(s.eVar72) {s.prop72="D=v72"};
